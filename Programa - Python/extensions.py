...
def installDotNet35(version):
    ...
    arguments = ['DISM', '/Online', '/Enable-Feature', '/FeatureName:NetFx3', '/All',
               '/LimitAccess', f'/Source:1 - Complementos\\NetFramework35\\{version}', '/NoRestart']
...
def installDotNet7():
    print('Instalando Net Framework 7')
    programPath = ('1 - Complementos\\NetFramework7.exe')
    arguments = [programPath, '/s']

    command = run(arguments)

    if command.returncode != 0:
        print('Erro ao instalar Net Framework 7')
        print(command.returncode)
    else:
        print('Net Framework 7 instalado com sucesso!\n')


def installJava():
    print('Instalando Java 8...')
    programPath = '1 - Complementos\\Java.msi'
...
def installCRuntime():
    ...
    programPath = '1 - Complementos\\VisualC++\\'
    errorList = []
...
def installDirectX():
    ...
    programPath = '1 - Complementos\\DirectX\\DXSETUP.exe'
